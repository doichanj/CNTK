...
BOOST_AUTO_TEST_CASE(CheckGetCurrentCursorForRandomizers)
{
    ...
    auto test = [](SequenceEnumeratorPtr r, size_t epochSize)
    {
        ...
        BOOST_CHECK_EQUAL(firstCursor[L"minibatchSourcePosition"].Value<size_t>(), firstEpoch.size());

        auto secondEpoch = ReadFullEpoch(r, epochSize, 1);
        auto secondCursor = r->GetState();
        BOOST_CHECK_EQUAL(secondCursor[L"minibatchSourcePosition"].Value<size_t>() - firstCursor[L"minibatchSourcePosition"].Value<size_t>(), secondEpoch.size());

        auto thirdEpoch = ReadFullEpoch(r, epochSize, 2);
        auto thirdCursor = r->GetState();
        BOOST_CHECK_EQUAL(thirdCursor[L"minibatchSourcePosition"].Value<size_t>() - secondCursor[L"minibatchSourcePosition"].Value<size_t>(), thirdEpoch.size());

        auto anotherSecondEpoch = ReadFullEpoch(r, epochSize, 1);
        auto anotherSecondCursor = r->GetState();

        BOOST_CHECK_EQUAL(anotherSecondCursor[L"minibatchSourcePosition"].Value<size_t>(), secondCursor[L"minibatchSourcePosition"].Value<size_t>());
    };
